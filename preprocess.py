...
def preprocess_eeg_segments(signals,original_sample_rate, target_sample_rate,window_length=10000):
    signals = resample_signals(signals,original_sample_rate,target_sample_rate)
    signals = segment_signals_list(signals,window_length)
    FT_signals_list = []
    for signal in signals:
        FT_signal,_ = FFT(signal,window_length)
        FT_signals_list.append(FT_signal)
    return FT_signals_list